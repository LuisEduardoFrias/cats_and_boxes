        ...
        <div key={index} tabindex={index} className={`tile-container ${piece.name} ${getSelectPiece}`} style={tileStyle}  >
            ...
            {piece.tiles.map((obj, i) => rotation === obj.rotation ?
                obj.tiles.map((o, ind: number) => <Pieces
                    ...
                    catsInBoxes={getImgBox(boxChangeImg, o, point, rotation, piece.name, ind)}
                    refr={setNodeRef}
                    pieceName={piece.name}
                    rotation={rotation}
                    isSelected={isSelected}
                    onclick={() => dispatch({ type: "SelectTile", tile: { name: piece.name, rotation, point } })}
                    index={ind}
                    listeners={listeners}
...
function getImgBox(boxChangeImg, obj, point, rotation, pieceName, ind: number) {
    ...
    if (boxChangeImg.length > 0 && obj.img.includes("box")) {
        ...
        catsInBoxes = boxChangeImg?.find(e => e.name === pieceName && e.indexBox === boxIndex);
    }

    if (catsInBoxes)
        return catsInBoxes?.img;
...
//tile intermos
function Pieces({ piece, catsInBoxes, selectAllforfilter, refr, isSelected, rotation, pieceName, onclick, index, listeners, attributes }: TPieces) {
    ...
    return (
        <div style={stylePiece} ref={refr} className={cn} onClick={onclick} {...list} {...attr} >
            {piece.img.includes("box") ? <img key={index} src={`/images/${catsInBoxes ?? piece.img}.png`} /> : null}
        </div>
    )
}
...
function Control({ tile_name, release, rotation, orientation }: { tile_name: string, release: boolean, rotation: 0 | 90 | 180 | 270, orientation: number }) {
    ...
    return (
        <div className="tile-control-container" style={controlStyle} >
            ...
            <button disabled={!release} className={`tile-control-btn tile-control-insert`} onClick={() => dispatch({ type: "DeselectTile", tile: undefined })}>
